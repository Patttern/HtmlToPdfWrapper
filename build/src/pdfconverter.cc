  ...
  for (QList<PageObject>::iterator i=objects.begin(); i != objects.end(); ++i) {
    ...
    if (!s.header.htmlUrl.isEmpty() ) {
      ...
      if (settings.margin.top.first == -1) {
        ...
        o.measuringHeader = &measuringHFLoader.addResource(MultiPageLoader::guessUrlFromString(s.header.htmlUrl), s.load)->page;
      } else {
    ...
    if (!s.footer.htmlUrl.isEmpty()) {
      ...
      if (settings.margin.bottom.first == -1) {
        ...
        headerHeightsCalcNeeded = true;
        o.measuringFooter = &measuringHFLoader.addResource(MultiPageLoader::guessUrlFromString(s.footer.htmlUrl), s.load)->page;
      } else {
        // or just set static values
...
/**
 * @brief Prepares printing out the document to the pdf file
 * @param ok ???
 */
...
/**
 * @brief Replace some variables in a string used in a header or footer
